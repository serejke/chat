 ...
 * initializes it, and returns the Chat plus the Telegram adapter. Used by both
 * the real-bot entry (src/index.ts) and the emulator entry
 * (src/index.emulated.ts) — everything except where the adapter points is
 * identical.
 */
...
export interface StartedBot {
  chat: Chat;
  /**
   * Exposed so callers can stop the polling loop on shutdown. Chat.shutdown()
   * cannot do it — the Telegram adapter implements no disconnect() hook, so
   * Chat skips it and the long-poll keeps running.
   */
...
export async function startBot(config: StartBotConfig): Promise<StartedBot> {
